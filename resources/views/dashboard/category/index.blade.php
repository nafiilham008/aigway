<div :class="{ 'ml-56': sidebarOpen, 'ml-0': !sidebarOpen }"
    ...
    <div class="bg-white p-6 rounded-lg">
        <h1 class="text-[#272B30] text-lg font-semibold font-vietnam mb-1">Kategori</h1>
        <h1 class="text-[#737373] text-xs font-vietnam mb-5">
            {{ $categoryCount != 0 ? $categoryCount . ' Kategori tersedia' : 'Belum ada kategori tersedia' }}
        </h1>
        <div class="flex justify-end mb-3">
            <button wire:click="$emit('modalOpen')"
                class="px-8 py-2 rounded-md button text-sm text-white">Tambah</button>
        </div>
        {{-- <button data-modal-toggle="add-category" class="px-8 py-2 rounded-md button text-sm text-white">Tambah</button> --}}
        ...
        <livewire:dashboard.category.component.category-datatable />

        <livewire:dashboard.category.component.modal-create />
