...
@endsection
<div class=" py-5">
    <div class="lg:px-24 px-5 flex gap-[10px] items-center">
        <a href="/katalog" class="font-vietnam font-medium text-base text-[#272727]/50">Katalog</a>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <defs>
                <linearGradient id="paint0_linear_187_865" x1="13.2427" y1="9.99984" x2="-5.3083" y2="9.99984"
        ...
        <h1 class="font-vietnam font-semibold text-base text-black">{{ $detailProduct->name }}</h1>
    </div>
    <hr class="mb-7 mt-4 h-[0.5px] border-[#D1D1D1] border">

    <div class="flex lg:flex-row flex-col justify-center lg:px-0 px-5">
        <div class="flex lg:flex-none justify-center lg:mb-0 mb-8">
            <div class="column">
                <div class="flex justify-center">
                    @if (!empty($detailProduct->photo))
                        <img id="featured" src="{{ asset('storage/' . $detailProduct->photo[0]) }}" class="mb-6">
                    @endif
                </div>

                <div class="flex lg:flex-none justify-center">
                    <div id="slide-wrapper">
                        <button id="slideLeft">
                            <svg width="67" height="67" viewBox="0 0 67 67" fill="none"
                                ...
                                <defs>
                                    ...
                                    <linearGradient id="paint0_linear_204_478" x1="28.8518" y1="31.5"
                                        x2="49.7217" y2="31.5" gradientUnits="userSpaceOnUse">
                                        <stop offset="0.114583" stop-color="#BA2B60" />
                                        <stop offset="0.65625" stop-color="#E9C6CC" />
                                        <stop offset="0.65635" stop-color="#FFB9C5" />
                                        <stop offset="0.90625" stop-color="#C7ECE2" />
                        ...
                        <div id="slider">
                            @if (!empty($detailProduct->photo))
                                @foreach ($detailProduct->photo as $index => $photo)
                            ...
                            <svg width="67" height="67" viewBox="0 0 67 67" fill="none"
                                ...
                                <g filter="url(#filter0_d_204_481)">
                                    <circle cx="22.5" cy="22.5" r="22.5"
                                        transform="matrix(-1 0 0 1 55 9)" fill="white" />
                                </g>
                                <path
                                ...
                                <defs>
                                    <filter id="filter0_d_204_481" x="0" y="0" width="67"
                                        height="67" filterUnits="userSpaceOnUse"
        ...
        <div class="w-full lg:px-40">
            <h1 class="lg:text-[40px] text-[30px] font-semibold font-vietnam mb-2">{{ $detailProduct->name }}</h1>
            <h1 class="text-base text-[#272727]/80 font-vietnam mb-[30px]">
                {{ $detailProduct->category ? $detailProduct->category->label : 'Lainnya' }}
            </h1>
            <h1 class="lg:text-[32px] text-[25px] font-semibold font-vietnam mb-2">
                {{ $detailProduct->price ? 'IDR. ' . number_format($detailProduct->price, 0, ',', '.') : ' - ' }}
            </h1>
            ...
            <div class="accordion flex flex-col items-center justify-center">
                ...
                <div class="w-full">
                    ...
                    <!-- Tambahkan atribut checked -->
                    <label for="panel-1"
                        class="relative block bg-transparent text-black text-base lg:text-lg p-4 border-t border-[#D1D1D1] font-medium cursor-pointer">
                        Tentang Produk
                    </label>
                    <div class="accordion__content overflow-hidden bg-grey-lighter">
                        <p class="lg:text-sm text-xs font-vietnam text-[#272727] p-4">
                ...
                <div class="w-full">
                    ...
                    <div class="accordion__content overflow-hidden bg-grey-lighter w-full">
                        <div class="p-4 w-full">
                            @if ($detailProduct->benefits)
                                @foreach (json_decode($detailProduct->benefits) as $benefit)
                                    <div class="flex gap-5 mb-[15px]">
                                        ...
                                        <h1 class="lg:text-base text-sm  font-vietnam text-[#272727]">
                                            {{ $benefit }}</h1>
                                    </div>
                                @endforeach
                            @else
                    ...
                    @if ($detailProduct->additional_information)
                        <div class="w-full">
                            <input type="checkbox" name="panel" id="panel-3"
                                class="hidden">
                            <label for="panel-3"
                                class="relative block bg-transparent text-black text-lg p-4 border-t border-[#D1D1D1] font-medium cursor-pointer">Keterangan
                                Tambahan</label>
                            <div class="accordion__content overflow-hidden bg-grey-lighter">
                                <p class="accordion__body p-4">{{ $detailProduct->additional_information }}</p>
                            </div>
                        </div>
                    @endif
                </div>
